...
directions to walk (e.g. ['n', 's', 'w', 'e']). You always walk only a single block for each letter
(direction) and you know it takes you one minute to traverse one city block, so create a function
that will return true if the walk app gives you will take you exactly ten minutes
...
result = is_valid_walk(['s', 'e', 'w', 'n', 'n', 's', 'e', 'w', 'n', 's'])
print(result)
